...
public class UserController {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> credentials) {
        ...
        if (email == null || password == null) {
            return ResponseEntity.status(401).body(Map.of("message", "Please provide your email and password"));
        }
    ...
    @GetMapping("/")
    public ResponseEntity<?> getSingleUser(@RequestParam Map<String, String> user) {
        try {
            User singleUser = userService.getSingleUser(Long.parseLong(user.get("user_id")));
